...
/* operation sequence, like "w", "dd", "df"
 * opseq表示一个操作基元，一段opstr被分成多个opseq来逐个处理。
 * 它叫做opunit似乎更好，opstr才改叫做opseq。就是vi_normal接收的那个ops。再说。
 */
struct opseq{
	union{
		int value;
		char byte[4];		//so, big endian machine is not supported here
	};
	int totlen;
};

/*@return 
 * opseq.totlen = -1		illegal op sequence
 * opseq.totlen = 0			遇到终点了。　最好不要让它这样返回，在上层就检测结尾
 * opseq.byte[], 能容纳的操作码序列最长为４。这已经很够了。我见到的最长也就２
 				 像i123，只有i是操作码。　像/123<CR>，只有/是操作码。
				 像df1,只有df是操作码。
 */
static struct opseq taste_opseq(struct vi *vi){
	char *currop = vi->ops + vi->op_id;
	struct opseq opseq;

	opseq.value = *currop;	//第一次初始化，不要用opseq.byte[0] = *currop;
	switch(*currop){
		case 'd':
		case 'y':
				{
					if(vi->v_begin){		//这个vi出于选择模式
						opseq.totlen = 1;
						break;
					}

					opseq.byte[1] = currop[1];
					if(currop[1] == currop[0]){
						opseq.totlen = 2;
						break;
					}
					switch(currop[1]){
						case 'f':
							opseq.totlen = 3;	break;
						case 'w':	assert(0);
						case 'W':
						case '0':
						case '$':
							opseq.totlen = 2;	break;
						default:
							opseq.totlen = -1;	break;
					}
		...
		case 'a':	
				{
					char *content = currop + 1;
					int len = strlen_ex(content, STR_ESC);
					opseq.totlen = 1 + len;
					if(content[len] == ASCII_ESC) opseq.totlen += 1;
					else;	//遇到结尾的\0了。
		...
		case '?':
				{
					...
					int len = strlen_ex(pattern, STR_CR);	//此处CR和'\n'不同
															//的意义就出来了
					opseq.totlen = 1 + len + 1;	// the last '1' for <CR>
					break;
				}

		case 0:
					opseq.totlen = 0;		break;
		default:
					opseq.totlen = -1;		break;
	}
	return opseq;
...
/*
 * 没有count，则返回默认的1。　
 * 这样有潜在的风险，即，上层需要知道，这个１到底是缺省生成的，还是用户指定的。
 */
static int taste_count(char *opseq, char **endptr){
	if(opseq[0] == '0') return 1;		//0是opcode,不能做count prefix
	if(opseq[0] == '-') return 1;		//不接受负数的count prefix
	if(opseq[0] == ASCII_DEL) return 0;	//DEL键的ascii码才表示重复前缀是0	

	int count = strtol(opseq, endptr, 10);
	if(count == 0) return 1;	//用户没有输入命令前缀，
	return count;
}

//暂时只考虑两个
#define SEQ(a, b) ( tochar(a) + ( tochar(b) << 8 ))
...
//use f|, l|, w_, e_
/* > 对重复前缀的支持:
 *   1, 前缀N一经探测处，就记录在vi->N里。
 *	 2, 像$(i)中的i为０，那么展开后会变成127。vi对于变量性质的0是当做前缀处理。
 *	 3, 现在是对所有动作的重复前缀统一处理，虽然100G,100gg是跳到100行。以后再说
 *	 4, 支持重复前缀时注意程序的美感。至少目前，不会为了效率破坏代码的简洁
 *   5, 为了支持N=0，需要子层识别N=0，做dry run。即，解析掉一个命令字段，但不
 *      产生实际的效果。
 * > op必须以０结尾，而不是\n
 * > 暂时不提供vi->N。只用局部变量。
 */
bool vi_normal(struct vi *vi, char *ops){
	int len;
	bool ok;

	preprocess(ops);		//直接处理可以吗，这可是从lua stack传过来的，const ?
	//printf("preprocess done: %s\n", op);
	vi->ops = ops;
	vi->op_id = 0;

	//bool (*do_search)(struct vi *vi, char *pattern);	//TODO
	//暂时不做过多的错误反馈
	while(vi->ops[vi->op_id]){
		char *currop = vi->ops + vi->op_id;
		int N = taste_count(currop, &currop);
		vi->op_id = currop - vi->ops;

		//这是命令的解析阶段，会检测格式错误
		struct opseq opseq= taste_opseq(vi);	
		if(opseq.totlen == -1){
			ok = false;
			goto out;;	//刚才这个命令解析失败了
		}

		if(N < 1 ) goto take_effect_done;	//在此处支持N=0

		take_effect:							//开始opseq的运行阶段
		ok = 1;
		switch(opseq.value){
			case 'd':
							ok = vi_d(vi);				break;
			case SEQ(d, f):
							ok = vi_df(vi, currop[2]);				break;
			case SEQ(d, d):
							vi_dd(vi);					break;
			case SEQ(d, W):
							ok = vi_dW(vi);				break;
			...
			case 'y':		ok = vi_y(vi);				break;
			case SEQ(y, f):
							ok = vi_yf(vi, currop[2]);				break;	
			case SEQ(y, y):
							vi_yy(vi);					break;
			case SEQ(y, W):
							ok = vi_yW(vi);				break;
			case SEQ(y, w):	assert(0);
			case SEQ(y, 0):
							vi_y0(vi);					break;
			case SEQ(y, $):
							vi_y$(vi);					break;
			...
			case 'W':		ok = vi_W(vi);		break;
			case 'w':		ok = vi_w(vi);		break;
			case 'B':		ok = vi_B(vi);		break;
			case 'E':		ok = vi_E(vi);		break;
			case 'e':		ok = vi_e(vi);		break;
			case 'b':		ok = vi_b(vi); 		break;

			case 'f':		ok = vi_f(vi, currop[1]);	break;
			case 'F':		ok = vi_F(vi, currop[1]);	break;
			case 'm':		ok = vi_m(vi, currop[1]);	break;
			case '`':		ok = vi_tom(vi, currop[1]); break;

			case 'v':		vi_v(vi);		break;	
			case 'G':		vi_G(vi);		break;
			case 'p':		ok = vi_p(vi);	break;
			case 'x':		vi_x(vi);		break;

			case '0':		vi_0(vi);			break;
			case '$':		vi_$(vi);			break;
			case 'h':		ok = vi_h(vi);	break;
			case 'j': 		ok = vi_j(vi);	break;
			case 'k':		ok = vi_k(vi);	break;
			case 'l':		ok = vi_l(vi);	break;
			case '^':		ok = vi_xor(vi);	break;
			case '%':		ok = vi_percent(vi);	break;

			case 'n':		ok = vi_n(vi);	break;
			case 'N':		ok = vi_N(vi);	break;
			case ASCII_ESC: 				break;

			case 'o':		vi_o(vi);
			case 'i':		len = vi_i(vi, currop + 1);		goto insert_done;
			case 'a':		len = vi_a(vi, currop + 1);		goto insert_done;
			insert_done:
							if(len < 0) ok = false;
							break;
			//去细看这两个search函数，它们怎么就直接把pattern当做字符串了。
			//最后一个<CR>不考虑吗。即使这样，也还要支持<CR>的后续操作。
			case '/':
							ok = vi_search_foward(vi, currop + 1);
							break;
			case '?':
							ok = vi_search_backward(vi, currop + 1);
							break;
		...
		take_effect_done:
		if(!ok)	goto out;	//刚才这个命令运行失败了
		N --;
		if(N >= 1) goto take_effect;	//重复刚才的操作直到N次

		vi->op_id += opseq.totlen;			//进入下一段指令序列
	}

	out:
	if(!__silent) vi_print(vi, ok);
	return ok;
}

/*register index:'a', 'b'...*/
//下面的assert都是必要的，因为是严重错误，用户肯定写错了代码
//或者抛出红色的不可屏蔽警告也可以。
bool vi_tom(struct vi *vi, char rid){
	if(vi_mvalid(vi, rid)){
		struct mark_register *r = &vi->m_regs[rid];
		assert(r->currl >= 0 && r->currl <= vi->lmax);
		assert(r->curroff >= 0 && r->curroff <= VI_CURRL_LEN(vi));
		vi->currl = r->currl;
 ...
 * 目前版本的标记记录的是行号和列号(行内偏移)，他不会跟着内容变动。
 */
