...
4,如果fs不能直接应答i80，而是把系统呼叫转发给具体的文件系统，那么先让呼叫者sleep。
*/
...
int askfs(int command,
	...
	switch(g_dp[pfd->device][pfd->partition].sys_id){
		case SYSID_LINUX:
			cmd->handler_p = __ext_pcb;
			if(__ext_pcb->msg_type==MSGTYPE_USR_ASK) sleep_active(__ext_pcb);
			active_sleep(current, MSGTYPE_FS_DONE, 0);
			schedule();
			break;
		case SYSID_FAT16:
			break;
		...
		case SYSID_NTFS:
			break;
		case SYSID_EXTEND:
			spin("entend partition is not partition..\n");
			return 0;
		default:
			spin("askfs:unknown partition partition sys_id...\n");
			return 0;
	}
...
FS_COMMAND*is_there_cmd_wait(void){
	int i;
	for(i=0;i<MAX_CMD;i++){
		if(empty_cmds[i].handler_p==current&&empty_cmds[i].command!=COMMAND_NULL) break;
	}
