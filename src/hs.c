...
//	__asm__("cli");//hs运行时候不响应中断，他会主动放弃时间片,内核进程受到信任
	...
	//	__asm__("cli");//这儿必须关中断，hs向磁盘驱动器写端口之后，要确保休眠。如果不关中断，来不及休眠，磁盘中断可能就送上来了。
		...
		if(cmd.command==COMMAND_IDENTIFY){//hs起初发送的是identify请求
			kthread_sleep(MSGTYPE_HD_DONE,0);
...
//					oprintf("hs:waked by hd-irq\n");
					...
					in_byte(REG_STATUS);//activate the interrupt enable bit
					cmd.reg_count--;
					kthread_sleep(MSGTYPE_HD_DONE,0);//等待磁盘向磁缓读入一个扇区
				}
			}
			cmd.count-=reg_count;
			cmd.lba+=reg_count;
...
void askhs(int command,int lba,int count,char*buf){
	while(__hs_pcb->msg_type != MSGTYPE_FS_ASK){
		oprintf("fs:can not command hs now,sleep on MSGTYPE_HS_READY,msg_type=%u\n",__hs_pcb->msg_type);
		kthread_sleep(MSGTYPE_HS_READY,0);
	}
	//send command-details to hs,and sleep for hs done
	hs_cmd_init(lba,count,command,buf);
	sleep_active(__hs_pcb);
	kthread_sleep(MSGTYPE_HS_DONE,0);
}
