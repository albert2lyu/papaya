...
int main(void){
	char cmd[100];
	sprintf(cmd,"dd if=../bin/kernel.elf of=../cmd/400m.img bs=512 conv=notrunc seek=%u",kernel_image_start_sector-1);
	system(cmd);

	unsigned char boot_flag[]={0x55,0xaa};
	int fd_sys=open("../cmd/400m.img",1+1,0);
	if(fd_sys==-1){
	...
	system("dd if=../bin/boot.bin of=../cmd/400m.img bs=512 conv=notrunc seek=0");
	/**write back dpt*/
	lseek(fd_sys,446,0);
